    ...
    for (size_t i = 0; i < n_frames; i++) {
        ...
        if (i != n_frames - 1) {
            ...
            if (semi_pos != std::string::npos) {
                ...
                if (!inline_lambdas) {
                    if (jl_is_expr(outer_linfo->ast))
                        inline_lambdas = jl_exprarg(outer_linfo->ast, 3);
                    else
                        inline_lambdas = jl_uncompress_ast_(outer_linfo, outer_linfo->ast, 1);
                }
                jl_svec_t *loc = (jl_svec_t*)jl_cellref(inline_lambdas, inl_idx-1);
                frame->linfo = (jl_lambda_info_t*)jl_svecref(loc, 0);
                frame->specSig = (jl_value_t*)jl_svecref(loc, 1);
            }
        }
